...
interface JobDetailsDialogMobileProps {
  open: boolean;
  onClose: () => void;
  job: Job;
...
function JobDetailsDialogMobile({
  ...
  job,
  getStatusIndicator,
}: JobDetailsDialogMobileProps) {
  // Состояния компонента
  const [activeTab, setActiveTab] = useState<
    "executions" | "schedule" | "config" | "events"
  >("executions");
  const {
    formatDateTime,
    handleStopClick,
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            Создано: {job.created_at && formatDateTime(job.created_at)}
          </Typography>
          <Typography
            variant="body2"
            sx={{
              color:
                statusColors[job.build_status as keyof typeof statusColors] ||
                "black",
              textAlign: "center",
              mt: 1,
            }}
                    ...
                    <List>
                      {Object.entries(events).map(([dateTime, log], index) => (
                        ...
                        >
                          ...
                          <Typography variant="body2">{String(log)}</Typography>
                        </Paper>
                      ))}
                    </List>
                  ) : (
