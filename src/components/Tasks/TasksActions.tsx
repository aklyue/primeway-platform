...
})<ActionIconButtonProps>(({ theme, colorvariant }) => ({
  transition: "all 0.2s ease",
  "&:hover": {
    transform: "scale(1.1)",
    backgroundColor:
...
interface TasksActionsProps {
  job: Job;
  onLogsClick: (job: Job, jobExecutionId?: string | null) => void;
  onBuildLogsClick: (job: Job) => void;
  onDownloadArtifacts: (job: Job) => void;
  onStopClick: (job: Job) => void;
  showStartButton: boolean;
  showLogsArtButton: boolean;
  displayMode: string;
