...
import { Dataset } from "../../../../types/datasets/Datasets";
...
export default function TrainPage({ isMobile, isTablet }: TrainPageProps) {
  ...
  const [maxSeqLen, setMaxSeqLen] = useState<string | number>("2048");
  const [batchSize, setBatchSize] = useState<string | number>("2");
  const [gradAccum, setGradAccum] = useState<string | number>("4");
  const [epochs, setEpochs] = useState<string | number>("1");
  const [learningRate, setLearningRate] = useState<string | number>("0.0002");
  const [weightDecay, setWeightDecay] = useState<string | number>("0.01");
  const [seed, setSeed] = useState<string | number>("42");
  const [loraR, setLoraR] = useState<string | number>("16");
  const [loraAlpha, setLoraAlpha] = useState<string | number>("16");
  const [loraDropout, setLoraDropout] = useState<string | number>("0");
