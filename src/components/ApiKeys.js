...
function ApiKeys() {
  ...
  const handleDeleteApiKey = async (tokenId) => {
    const confirmed = window.confirm('Вы уверены, что хотите удалить этот API ключ?');
    if (!confirmed) return;
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" gutterBottom>
        API-ключи позволяют вам проверять подлинность запросов к нашему API.
        Вы можете создать новый API-ключ и управлять существующими.
        Пожалуйста, обратите внимание, что секретный ключ будет показан только один раз после его создания. Храните его в безопасности!
      </Typography>
                  ...
                  <TableRow key={key.id}>
                    ...
                    <TableCell>
                      <IconButton
                        ...
                        aria-controls={menuItemId === key.id ? 'action-menu' : undefined}
                        aria-haspopup="true"
                      >
                        <MoreVertIcon />
                      </IconButton>
