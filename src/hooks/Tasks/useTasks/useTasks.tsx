...
export const useTasks = ({ authToken, currentOrganization }: useTasksProps) => {
  const [allJobs, setAllJobs] = useState<Job[]>([]);
  const [jobs, setJobs] = useState<Job[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [jobTypeLoading, setJobTypeLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);

  // Состояния фильтров
  const [triedSwitchJobType, setTriedSwitchJobType] = useState<boolean>(false);
  ...
  const [executionsModalOpen, setExecutionsModalOpen] =
  ...
  const selectedJobType = useAppSelector(
    (state) => state.tasksFilters.selectedJobType
  );
  const isScheduledFilter = useAppSelector(
  ...
  const handleTaskClick = useCallback((job: Job) => {
    setCurrentJob(job);
  ...
  const handleLogsClick = useCallback(
    (job: Job, jobExecutionId: string | null = null) => {
  ...
  const handleBuildLogsClick = useCallback(
    (job: Job) => {
      ...
      axiosInstance
        ...
        .catch((error) => {
          console.error(
            `Ошибка при получении логов сборки для задачи ${job.job_id}:`,
            error
          );
          const errorMessage =
            error.response?.data?.detail ||
  ...
  const handleExecutionsClick = useCallback(
    (job: Job) => {
      ...
      axiosInstance
        ...
        .catch((error) => {
          console.error(
            `Ошибка при получении выполнений для задачи ${job.job_id}:`,
            error
          );
  ...
  const handleScheduleClick = (job: Job) => {
    ...
    axiosInstance
      ...
      .catch((error) => {
        console.error(
          `Ошибка при получении расписания для задачи ${job.job_id}:`,
          error
        );
        const errorMessage =
  ...
  const getStatusIndicator = (job: Job) => {
    ...
    const color = statusColors[status as JobStatus] || "grey";

    if (status && ["creating", "pending", "provisioning"].includes(status)) {
      return <CircularProgress size={15} thickness={5} sx={{ color: color }} />;
    } else if (status) {
      return <FiberManualRecord sx={{ color: color, fontSize: 15 }} />;
    } else {
      return null;
    }
  };
