...
function SpecificModel({
  ...
  const [confirmLaunchOpen, setConfirmLaunchOpen] = useState<boolean>(false);
  const [flags, setFlags] = useState<AdditionalFields[]>([]);
  const [args, setArgs] = useState<AdditionalFields[]>([]);
  ...
  const handleFlagsChange = useCallback((newFlags: AdditionalFields[]) => {
    setFlags(newFlags);
  }, []);

  const handleArgsChange = useCallback((newArgs: AdditionalFields[]) => {
    setArgs(newArgs);
  }, []);

  const handleModelConfigChange = useCallback((newConfig: ModelConfig) => {
