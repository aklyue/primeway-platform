...
describe("GPUList Component", () => {
  ...
  test("Отображает индикатор загрузки во время загрузки данных", () => {
    render(<GPUList isMobile={false} isTablet={false} />);
    expect(screen.getByRole("progressbar")).toBeInTheDocument();
  });

  test("Отображает индикатор загрузки во время загрузки данных", () => {
    render(<GPUList isMobile={true} isTablet={true} />);
    expect(screen.getByRole("progressbar")).toBeInTheDocument();
  });
  ...
  test("Отображает список GPU после успешной загрузки данных", async () => {
    ...
    (axiosInstance.get as jest.Mock).mockResolvedValueOnce({ data: gpuData });
    ...
    gpuData.forEach((gpu) => {
      ...
      const withinCard = within(cardElement as HTMLElement);
  ...
  test("Копирует имя GPU при клике на карточку и отображает Snackbar", async () => {
    ...
    Object.assign(navigator, {
      clipboard: {
        writeText: jest.fn().mockResolvedValueOnce("text copied"),
      },
    });
