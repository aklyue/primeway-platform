...
import { Organization, User } from "../../store/slices/authSlice";
import { CreditEntry, Credits, DailyCreditUsage } from "../../types";
...
export const useBillingActions = ({
  ...
  const [creditUsagePerDay, setCreditUsagePerDay] = useState<
    DailyCreditUsage[]
  >([]); // Данные для графика расходов
  const [creditPurchasesData, setCreditPurchasesData] = useState<
    DailyCreditUsage[]
  >([]); // Данные для графика покупок
        ...
        .then(
          ([
            ...
            const purchasesData = (
              ...
              return {
                date,
                credits: parseFloat(tx.credits.toString()),
              };
            });

            // Группируем покупки по дате
            const purchasesByDate: Record<string, number> = {};
            purchasesData.forEach((purchase: DailyCreditUsage) => {
  ...
  const handlePaymentError = (error: Error | PaymentResult) => {
