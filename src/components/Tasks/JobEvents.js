...
function JobEvents({ jobId }) {
  ...
  const fetchEvents = async () => {
    ...
    try {
      const response = await axiosInstance.get('/jobs/get-events', {
        params: { job_id: jobId },
      });
                ...
                return (
                  <Paper
                    ...
                    sx={{
                      ...
                      p: 2,
                      mb: 2,
                      ...(isValidLevel ? eventLevelStyles[level] : {}),
                    }}
                    key={index}
                  >
                    <Typography variant="subtitle1" sx={{ fontWeight: '600', fontSize:'12px', opacity: 0.5 }}>
                      {formatDateTime(event.timestamp)} <strong>{event.level.toUpperCase()}</strong>
                    </Typography>
