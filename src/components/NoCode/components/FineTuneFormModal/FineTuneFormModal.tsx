...
interface FineTuningJobFormModalProps {
  open: boolean;
  onClose: () => void;
}

/* ───────────────────────── COMPONENT ───────────────────────── */
export default function FineTuningJobFormModal({
  open,
  onClose,
}: FineTuningJobFormModalProps) {
          ...
          <Select value={selectedGpu} onChange={handleGpuChange} label="GPU">
            {Object.keys(AVAILABLE_GPUS).map((k) => (
              <MenuItem key={k} value={k}>
                {AVAILABLE_GPUS[k as keyof typeof AVAILABLE_GPUS].name} –{" "}
                {AVAILABLE_GPUS[k as keyof typeof AVAILABLE_GPUS].memoryInGb} GB
              </MenuItem>
            ))}
          </Select>
        </FormControl>
