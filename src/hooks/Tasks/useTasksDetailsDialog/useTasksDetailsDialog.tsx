...
interface useTasksDetailsDialogProps {
  open: boolean;
  job: Job;
}
...
export const useTasksDetailsDialog = ({
  ...
  const [configEditing, setConfigEditing] = useState<boolean>(false);

  // Состояния для кнопок и логов
  const [logsLoading, setLogsLoading] = useState<boolean>(false);
  const [buildLogsLoading, setBuildLogsLoading] = useState<boolean>(false);
  const [buildLogsModalOpen, setBuildLogsModalOpen] = useState<boolean>(false);
  ...
  const initialExecutionsLoadRef = useRef<boolean>(true);
  const initialSchedulesLoadRef = useRef<boolean>(true);
  const initialConfigLoadRef = useRef<boolean>(true);
  ...
  const formatJobExecutionId = (id: string) => {
    if (!id) return "N/A";
  ...
  // Функции-обработчики
  const handleLogsClick = (execution: Execution | null = null) => {
  ...
  const handleCardStopClick = (job: Job) => {
  ...
  const fetchExecutions = async () => {
    ...
    try {
      const response = await axiosInstance.get<Job[]>("/jobs/executions", {
        params: { job_id: job.job_id },
      });
  ...
  const fetchSchedules = async () => {
    ...
    try {
      const response = await axiosInstance.get<AnySchedule[]>(
        "/jobs/get-schedules",
        {
          params: { job_id: job.job_id },
        }
  ...
  const fetchConfig = async () => {
    ...
    try {
      const response = await axiosInstance.get<JobConfig>("/jobs/get-config", {
        params: { job_id: job.job_id },
      });
  ...
  const renderScheduleDetails = (schedule: AnySchedule) => {
  ...
  const removeWorkdayWindow = (index: number) => {
    setNewSchedule((prev) => {
  ...
  const removeWeekendWindow = (index: number) => {
    setNewSchedule((prev) => {
  ...
  const handleSpecificDayWindowsChange = (
    index: number,
    windows: TimeWindow[]
  ) => {
    setNewSchedule((prev) => {
      const specific_days = [...prev.specific_days];
      specific_days[index].time_windows = windows;
      return { ...prev, specific_days };
    });
  };
  ...
  const handleApiError = (error: AxiosError, defaultMessage: string) => {
    ...
    const errorDetail = error.response?.data as ApiErrorResponse;
  ...
  const handleEditSchedule = (schedule: AnySchedule) => {
